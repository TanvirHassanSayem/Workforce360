...
from employee_information.models import Department, Position, Employees, Absence
...
def salary(request):
    employee_list = Employees.objects.all()
    total = Employees.objects.aggregate(total_salary=Sum('salary'))
    amount = 205000.0
    profit = amount - total['total_salary']
    context = {
        'page_title':'Salary',
        'employees':employee_list,
        'total': total,
    ...
    return render(request, 'employee_information\SalaryManage.html', context)
...
@login_required
def mark_present(request, employee_code):
    employee = Employees.objects.get(code=employee_code)
    ...
    return redirect('attendence')  # Replace 'attendance' with the name of your attendance view or URL
